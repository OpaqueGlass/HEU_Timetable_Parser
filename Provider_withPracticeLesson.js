async function scheduleHtmlProvider(iframeContent = "", frameContent = "", dom = document) {
    ...
    if (check == "查看课表"){
        ...
        result = "<div id=\"practiceDelay\">" + userInput + "</div>*#*##*#*" + theroyTable + "*#*##*#*" + practiceTable;
    }else if (check == "学期理论课表"){
        console.log("认定为学期理论课表");
        await AIScheduleAlert({
            titleText: '未打开工程实践课表', // 标题内容，字体比较大，不传默认为提示
            contentText: '将只导入理论课课表', // 提示信息，字体稍小，支持使用``达到换行效果，具体使用效果建议真机测试
            confirmText: '继续', // 确认按钮文字，可不传默认为确认
        });
        let ifrs = dom.getElementById("Form1");
        result = " *#*##*#*" + ifrs.innerHTML;
    }else{
        await AIScheduleAlert({
